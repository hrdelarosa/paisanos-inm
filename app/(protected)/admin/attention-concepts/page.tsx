...
export default function AttentionConcepts() {
  ...
  return <>
    <div className="mb-2">
      ...
      <p className="text-sm text-muted-foreground">Catálogo basado en el formato físico de cierre de jornada.</p>
    </div>
    <DataTable
      ...
      columns={[
        ...
        { key: 'name', label: 'Concepto', render: (type) => type.name },
        { key: 'requiresDescription', label: 'Requiere descripción', render: (type) => type.requiresDescription ? 'Sí' : 'No' },
        { key: 'isActive', label: 'Estado', render: (type) => <Badge variant={type.isActive ? 'default' : 'secondary'}>{type.isActive ? 'Activo' : 'Inactivo'}</Badge> },
      ]}
    />
  </>
}
